...
os.makedirs(os.path.join(opt.folder,"test_images/ep-{}/".format(opt.epoch)), exist_ok=True)
os.makedirs(os.path.join(opt.folder,"test_sample"), exist_ok=True)
...
model_name = "{}/models/epoch-{}.pth".format(opt.folder,opt.epoch)
...
generator.load_state_dict(torch.load(model_name)['generator'])
...
for i, sample in enumerate(tqdm(test_dataloader)):
    ...
    for i_d, gen_img in zip(sample['img_id'], gen_imgs):
        ...
        im.save(os.path.join(opt.folder,"test_images/ep-{}/{}".format(opt.epoch,i_d)))
...
save_image(gen_imgs.data, os.path.join(opt.folder,"test_sample/ep-{}_seed-{}.png".format(opt.epoch,opt.seed)), nrow=n_row, normalize=True)
